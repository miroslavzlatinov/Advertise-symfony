...
class ImageController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $query = $this->getDoctrine()->getRepository("AdvertBundle:Image")->createQueryBuilder("p");
        $query->select('p');
        $pagination = $paginator->paginate(
            $query->getQuery(),
            $request->query->getInt('page', 1), 6);
        try {
            return $this->render('image/index.html.twig', array('pagination' => $pagination));
        } catch (\Exception $e) {
            dump($e);
            exit;
        }
    ...
    {
        $image = new Image();
        $form = $this->createForm('AdvertBundle\Form\ImageType', $image);
    ...
    {
        $deleteForm = $this->createDeleteForm($image);
        $editForm = $this->createForm('AdvertBundle\Form\ImageType', $image);
